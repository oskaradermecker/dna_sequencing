...
{/*tampon (char*) : voir au dessus
	cette fonction renvoie les emplacement des élements du fichiers sous forme 
	* d'un vector de int*/
...
char* database::getHeadersFromDatabase(vector<int> offset,int index_header){
	...
	int end=offset[index_header+1];
	int i=begin;
	//Petit raccourci pour ne pas devoir décoder tous le phr
...
vector<char> database::getSequenceFromDatabase(vector<int> offset,int index_seq){
	 ...
	 * dans le fichier .psq
	 * index_header: int donnant l'indice de la sequence 
	 * 
	 * 
	 * */
