...
short query::separation(){
	...
	short i=0;
	while (text[i]!='\n'){
		//Tant qu'on ne détecte pas de passage à la ligne on incrémente de 1 our connaitre l'indice d'
		//où commence la séquence dans le fichier 
		i++;
	}
	return i;
}
...
int query::getLength(short separation){
	...
	while (i<strlen(text)){
		...
		{
			//Si on n'a pas pas de lettre ou un * on continue
			j++;
		}
		//cout << text[i];
		i++;
...
vector<char> query::getSequence(short begin_sequence,int length){
	/*renvoie la sequence de la protéine
	 * begin_sequence: short indiquant le début de la séquence
	 * length: short donne la longueur*/

	vector<char> sequence(length);//initialisation d'un vecteur sequence
	int j=0;
